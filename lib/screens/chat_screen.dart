...
class _ChatScreenState extends State<ChatScreen> {
  ...
  // Stranger room mein aa chuka hai ya nahi
  bool _hadOtherUser = false;
  ...
  void _listenForDisconnect() {
    _roomSubscription = FirebaseDatabase.instance
      ...
      // Current user room mein nahi hai
      ...
      // Jab 2 users room mein aa jayein
      if (users.length >= 2) {
        _hadOtherUser = true;
        return;
      }

      // Stranger pehle room mein tha aur ab chala gaya
      if (_hadOtherUser && users.length < 2) {
        await _roomSubscription?.cancel();
        await MatchService.leaveRoom(widget.roomId);
